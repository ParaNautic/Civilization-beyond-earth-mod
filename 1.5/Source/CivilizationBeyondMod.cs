...
namespace CivilizationBeyond
{
    public class CivilizationBeyondMod : Mod
    {
        public CivilizationBeyondMod(ModContentPack pack) : base(pack)
        {
            new Harmony("CivilizationBeyondMod").PatchAll();
        }
    }
    ...
    public class HediffComp_GiveHediffsInRange : HediffComp
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            {
                if (!item.RaceProps.Humanlike || item.Dead || item.health == null || !(item.Position.DistanceTo(parent.pawn.Position) <= Props.range) || !Props.targetingParameters.CanTarget(item))
                {
                    continue;
                }
